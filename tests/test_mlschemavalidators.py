...
class ValidatorsTestSuite(unittest.TestCase):
    ...
    def test_uuid_found(self):
        base_schema = MLSchema.create(SampleSchema.TEST.UUID)
        instantiated_schema = base_schema()
        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.UUID_VALID)
    ...
    def test_uuid_not_found(self):
        base_schema = MLSchema.create(SampleSchema.TEST.UUID)
        instantiated_schema = base_schema()
        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.UUID_INVALID)
        with self.assertRaises(ValidationError):
            instantiated_schema.load(yaml_submission)

    def test_uri_valid(self):
        base_schema = MLSchema.create(SampleSchema.TEST.URI)
        instantiated_schema = base_schema()
        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.URI_VALID_1)
        instantiated_object = instantiated_schema.load(yaml_submission)

        self.assertTrue(instantiated_object['endpoint'])

        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.URI_VALID_2)
        instantiated_object = instantiated_schema.load(yaml_submission)
    ...
    def test_uri_invalid(self):
        base_schema = MLSchema.create(SampleSchema.TEST.URI)
        instantiated_schema = base_schema()
        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.URI_INVALID_1)
        with self.assertRaises(ValidationError):
            instantiated_schema.load(yaml_submission)

        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.URI_INVALID_2)
        with self.assertRaises(ValidationError):
            instantiated_schema.load(yaml_submission)

    def test_regex_valid(self):
        base_schema = MLSchema.create(SampleSchema.TEST.REGEX)
        instantiated_schema = base_schema()
        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.REGEX_ALL_LETTERS)
        instantiated_object = instantiated_schema.load(yaml_submission)

        self.assertTrue(instantiated_object['all_letters'])

        yaml_submission = convert_to_yaml(SampleSubmissions.UNIT_TESTS.REGEX_ALL_NUMBERS)
        with self.assertRaises(ValidationError):
            instantiated_schema.load(yaml_submission)
